...
class PangianScraper(BaseScraper):
    async def search_jobs(self, role: str, location: str, **kwargs) -> List[Dict]:
        ...
        # Pangian Search URL
        # https://pangian.com/job-travel-remote/?search_keywords=Python
        url = f"https://pangian.com/job-travel-remote/?search_keywords={role.replace(' ', '+')}"
        ...
        try:
            ...
            logger.info(f"Found {len(cards)} potential job cards on Pangian")

            for card in cards:
                try:
                    title_el = card.locator("h3.job_listing-title").first
                    company_el = card.locator(".job_listing-company strong").first
